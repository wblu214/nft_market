...
## 5. Java / Spring Boot 对接建议（简单指引）

> 下面只是高层建议，你的后端同事可以用自己熟悉的 Web3 库实现。

1. **生成 Java 合约类**
   - 使用 Web3j 等工具，将上面的 ABI + 合约地址生成 Java 合约封装类，方便在 Spring Boot 中调用。

2. **核心调用流程**
   - 卖家挂单：
     - 用户使用钱包（或后端托管私钥）先对 NFT 合约做授权：  
       - ERC‑721：`approve(marketplaceAddress, tokenId)` 或 `setApprovalForAll`  
       - ERC‑1155：`setApprovalForAll(marketplaceAddress, true)`  
     - 后端调用 `NFTMarketplace.list(nftAddress, tokenId, amount, price)` 发送交易。
   - 卖家下架：
     - 后端调用 `NFTMarketplace.cancel(listingId)`。
   - 买家购买：
     - 后端调用 `NFTMarketplace.buy(listingId)`，并在交易中附带 `value = price` 的 ETH。

3. **事件订阅**
   - 订阅 `Listed`、`Cancelled`、`Sold` 三个事件：
     - 用于在数据库中同步订单状态变化。
     - 可以按区块高度顺序消费事件，保证幂等。

4. **错误处理与重试**
   - 如果交易因 gas 不足、授权不足、NFT 被转走等原因失败，区块链本身会 revert，交易 receipt 会标记失败。
   - 后端需要根据 receipt 状态和错误信息做重试/人工处理。

---

如需我再补一份针对 Web3j 的 Java 示例代码（比如如何构造 `buy` 交易、如何订阅 `Listed` 事件），可以在此文档基础上再单独出一份。  
